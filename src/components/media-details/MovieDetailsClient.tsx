...
export function MovieDetailsClient({ initialMovie, initialUserItem }: MovieDetailsClientProps) {
    ...
    // Update watch providers and release data whenever region changes
    useEffect(() => {
        let active = true;
        ...
        const updateData = async () => {
            ...
            // Fetch updated release dates if they weren't in the initial payload or to refresh
            try {
        ...
        // Don't re-run for initial TW region (already preloaded)
        if (region !== 'TW') {
            updateData();
        } else {
    ...
    return (
        <div className="min-h-screen bg-[#07070a] text-foreground pb-16">
            ...
            {/* Content Body */}
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 -mt-24 md:-mt-48 relative z-20">
                <div className="flex flex-col lg:flex-row gap-8 items-start">
                    ...
                    <div className="flex-1 space-y-8 w-full">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-wrap items-center gap-3">
                                ...
                                {initialMovie.rtScore && initialMovie.rtScore !== 'N/A' && (
                                    <div className="flex items-center gap-3 font-bold text-xs">
                                        <div className={`flex items-center gap-1 px-3 py-1 rounded-md ${initialMovie.rtStatus === 'fresh' ? 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-500' : 'bg-red-500/10 border border-red-500/20 text-red-500'}`}>
                                            <span className="text-sm">🍅</span>
                                            <span>{initialMovie.rtScore}</span>
                                        </div>
                                        {initialMovie.rtAudienceScore && (
                                            <div className={`flex items-center gap-1 px-3 py-1 rounded-md ${initialMovie.rtAudienceStatus === 'fresh' ? 'bg-blue-500/10 border border-blue-500/20 text-blue-500' : 'bg-orange-500/10 border border-orange-500/20 text-orange-500'}`}>
                                                <span className="text-sm">🍿</span>
                            ...
                            <div className="flex flex-wrap items-center gap-4 text-xs font-semibold text-foreground-muted border-t border-b border-border/20 py-3.5">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span>{localizedReleaseDate || details.release_date}</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 rounded-2xl border border-border/30 bg-background-elevated/20 p-5">
                            <div className="space-y-1">
                                <span className="text-[10px] font-bold uppercase tracking-widest text-foreground-muted">{t('revenue')}</span>
                                <p className="text-lg font-black text-foreground">{formatRevenue(details.revenue)}</p>
                            </div>
                            <div className="space-y-1">
                                <span className="text-[10px] font-bold uppercase tracking-widest text-foreground-muted">Budget</span>
                                <p className="text-lg font-black text-foreground">{formatRevenue(details.budget)}</p>
                            </div>
                        </div>
